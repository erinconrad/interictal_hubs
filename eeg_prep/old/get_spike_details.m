function details = get_spike_details(gdf,values,hf_values,fs)
...
for s = 1:size(gdf,1)
    ...
    % peak in unfiltered data
    [peak_unfilt,ind_unfilt] = max(abs(data(idx_to_look) - median(data)));

    % re-align index
    abs_ind = ind + idx_to_look(1) - 1;
    ind_unfilt = ind_unfilt + idx_to_look(1) - 1;
    ...
    details.peak_unfilt(s) = peak_unfilt;
    details.ind_unfilt(s) = ind_unfilt;
    details.ch(s) = ch;
...
details.gdf(keep==0,:) = [];
details.gdf(:,3) = [];
details.peak_unfilt(keep == 0) = [];
details.ind_unfilt(keep == 0) = [];
