function [change,no_change_ever] = alt_find_electrode_change_files(spikes,only_depth)
...
for f = 1:nfiles-1
    old_labels = clean_labels_2(spikes.file(f).block(1).chLabels(:,1));
    new_labels = clean_labels_2(spikes.file(f+1).block(1).chLabels(:,1));
