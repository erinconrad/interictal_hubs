function out = alt_get_gdf_details(p,spikes)
...
for c = nchanges % just do last one
    ...
    % remove those that are not present
    unchanged_idx(unchanged_idx == 0) = [];
    added_idx(added_idx == 0) = [];
