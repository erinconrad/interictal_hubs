function alt_pc_networks(whichPts)
...
out_folder = [results_folder,'alt/pc/'];
        ...
        for h = next_block:n_blocks
            ...
            if ~isempty(run_times)
                ...
                if length(bad) >= 0.5*length(intracranial_chs)
                    ...
                    [car_values,car_labels] = car_montage(values,pre_existing_idx,clean_labs);
         
                    run_chs = ismember((1:length(clean_labs))',which_chs);
                    
                    
                    %% Do filters
                    values = notch_filter(car_values,fs);
